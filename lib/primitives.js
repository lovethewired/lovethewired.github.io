...
export function Icon({ children, className, ...props }) {
  return (
    <div
      className={cn(
        // 'px-1 py-2 rounded text-white hover:text-secondary-400 transition-colors duration-200 cursor-pointer hover:scale-105',
        'text-white hover:text-secondary-400 transition-colors duration-200 cursor-pointer',
        className
      )}
      {...props}
    >
...
export function Card({ children, className, ...props }) {
  return (
    <div className={cn('bg-black/70 rounded-3xl shadow-lg px-4 py-2 border border-white/20', className)} {...props}>
      {children}
    </div>
  );
}
...
export function Portal({ children, ...props }) {
  ...
  return portal && createPortal(cloneElement(children, { onClose, ...props }), portal);
}
...
export function Modal({ children, onClose, className, ...props }) {
  ...
  const handleClick = (event) => {
    if (ref.current.isEqualNode(event.target)) setOpen(false);
    console.log(ref.current.isEqualNode(event.target));
  };
    ...
    <Portal>
      <div
        ...
        className={cn(
          ...
          'bg-gray-800/30 backdrop-blur-sm blur-xxl',
        ...
        {...props}
      >
        <div className="fixed max-w-[800px] flex items-center overflow-auto">{children}</div>
      </div>
    </Portal>
  );
}

export function Button({ disabled, variant = 'standard', className, ...props }) {
  return (
    <button
      disabled={disabled}
      className={cn(
        'rounded px-4 py-2 uppercase text-white select-none ',
        ...
        disabled
          ...
          : variant === 'fancy'
          ? 'bg-gradient-to-br from-primary-800 to-secondary-400 via-primary-600 bg-size-200 bg-pos-0 hover:bg-pos-100'
...
function MultilineTextArea({ value, onChange, className, ...props }) {
  const ref = useRef(null);
...
export function Input({
  ...
  return (
    <label className={cn('flex flex-col w-full', className)}>
      <span className={`text-sm px-1 ${disabled ? 'text-gray-400' : ' text-gray-200'}`}>{label}</span>
      <div
        className={cn(
          'inline-flex group border-0 border-white/20 rounded-t-md transition-colors duration-200',
          variant === 'outlined' ? 'rounded-md border-[1px]' : 'border-0 border-b-[1px]',
          invalid
            ? 'border-red-500'
            : disabled
            ...
            : 'hover:border-gray-100 focus-within:border-primary-600 focus-within:hover:border-primary-600 focus-within:shadow-[0_8px_12px_-14px_white]'
        )}
      >
        {multiline ? <MultilineTextArea {...allProps} /> : <input {...allProps} />}
        {children}
      </div>
    </label>
  );
...
export function LoadingButton({ loading, children, className, disabled, ...props }) {
  return (
    <Button
      className={`flex transition-all duration-2000 ${className ?? ''}`}
      disabled={disabled || loading}
      {...props}
    >
      {loading && (
        <svg
          className={`animate-spin -ml-1 mr-3 my-auto h-5 text-white ${loading ? 'w-5' : 'w-0'} `}
          fill="none"
          viewBox="0 0 24 24"
        >
...
export function Alert({ severity = 'info', children, onClose, className, autoHideDuration = 5000, ...props }) {
  const [open, setOpen] = useState(true);
  ...
  return (
    <Portal>
      <div className="fixed bottom-0 my-8 mx-8 z-30">
        <div
          {...props}
          className={cn(
            'relative px-6 py-4 max-w-lg mt-4 flex items-center rounded text-sm opacity-100 bg-black',
            fillColor,
            'transition-colors duration-500 animate__animated',
